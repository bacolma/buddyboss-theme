...
$filepath = locate_template( array(
            ...
            <form id="bb-courses-directory-form" class="bb-courses-directory" method="get" action="">
                ...
                <div class="grid-view bb-grid">
                    <div id="course-dir-list" class="course-dir-list bs-dir-list">
						<?php
                        if ( ! empty( $user_courses ) ) : ?><?php
							global $post;
							$_post = $post;
							?>
                            <ul class="bb-course-list bb-course-items bb-grid list-view <?php echo 'list' != $view ? 'hide' : ''; ?>" aria-live="assertive" aria-relevant="all">
								<?php foreach ( $user_courses as $course_id ) : ?><?php
                            ...
                            <ul class="bb-card-list bb-course-items grid-view bb-grid <?php echo 'grid' != $view ? 'hide' : ''; ?>" aria-live="assertive" aria-relevant="all"><?php
                                foreach ( $user_courses as $course_id ) : ?><?php
                                    $course = get_post( $course_id );
                                    $post   = $course;
                                    get_template_part( 'learndash/ld30/template-course-item' );
                                endforeach;
                        ...
                        <?php if ( $profile_pager['total_items'] >= 21 ) {
