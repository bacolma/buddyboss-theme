<?php
/**
 * The notification manager class for BuddyBoss SDK
 *
...
if ( ! class_exists( 'BuddyBoss_SDK_Notification_Manager' ) ) :
	/**
	 * Notification manager model for BuddyBoss SDK.
	 */
	class BuddyBoss_SDK_Notification_Manager {
		/**
		...
		/**
		 * @var BuddyBoss_SDK_Product Current product.
		 */
		private $product;
		/**
		 * @var array BuddyBoss_SDK_Feedback Feedbacks available.
		 */
		private $callbacks = array();

		/**
		 * BuddyBoss_SDK_Notification_Manager constructor.
		 *
		...
		function setup_notifications() {
			...
			if ( ( time() - $this->product->get_install_time() ) > self::NOTIFICATION_INTERVAL_HOURS * HOUR_IN_SECONDS ) {
				if ( $this->product instanceof BuddyBoss_SDK_Product && $this->callbacks && is_array( $this->callbacks ) ) {
