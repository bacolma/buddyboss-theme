...
if (!class_exists('\BuddyBossTheme\BuddyBossMultiPostThumbnails')) {
	...
	class BuddyBossMultiPostThumbnails {
		...
		public function enqueue_admin_scripts( $hook ) {
			...
			if (version_compare($wp_version, '3.5', '<')) {
				add_thickbox();
				wp_enqueue_script( "buddyboss-theme-mpt-featured-image", buddyboss_theme()->tpl_url() . '/assets/js/multi-post-thumbnails/multi-post-thumbails-admin.js' , array( 'jquery', 'media-upload' ) );
			} else { // 3.5+ media modal
				wp_enqueue_media( array( 'post' => ( $post_ID ? $post_ID : null ) ) );
				wp_enqueue_script( "buddyboss-theme-mpt-featured-image", buddyboss_theme()->tpl_url() . '/assets/js/multi-post-thumbnails/multi-post-thumbails-admin.js', array( 'jquery', 'set-post-thumbnail' ) );
				wp_enqueue_script( "buddyboss-theme-mpt-featured-image-modal", buddyboss_theme()->tpl_url() . '/assets/js/multi-post-thumbnails/media-modal.js', array( 'jquery', 'media-models' ) );
			}

			//wp_enqueue_style( "buddyboss-theme-mpt-admin-css", $this->plugins_url( 'css/multi-post-thumbnails-admin.css', __FILE__ ) );
		...
		public function admin_header_scripts() {
			...
			echo '<style>
				.edit-post-meta-boxes-area #sfwd-courses-course-cover-image:not(.closed) h2.hndle {
					border-bottom: 0;
				}
				.bb-multi-thumb .components-button.editor-post-featured-image__toggle {
					color: #444;
				}
				.bb-multi-thumb .components-button.editor-post-featured-image__toggle.has-cover {
					border: 0;
					padding: 0;
					background: transparent;
				}
				.bb-multi-thumb .components-button.editor-post-featured-image__toggle img {
					max-width: 247px;
					height: auto;
				}
				</style>';
		...
		private function post_thumbnail_html($thumbnail_id = null) {
			...
			$url_class = "components-button editor-post-featured-image__toggle";
				...
				$modal_js = sprintf(
					'var mm_%3$s = new MediaModal({
						...
						cb : function(attachment){
							BuddyBossMultiPostThumbnails.setAsThumbnail(attachment.id, "%2$s", "%1$s", "%4$s");
						}
					});',
			...
			if ($thumbnail_id && get_post($thumbnail_id)) {
				...
				if ( !isset($_wp_additional_image_sizes["{$this->post_type}-{$this->id}-thumbnail"])) {
					$thumbnail_html = wp_get_attachment_image( $thumbnail_id, 'full', false, $attr );
				} else {
